...
const Store = {
    ...
    updateHistorySession(index, updatedSession) {
        ...
        if (index === this.data.history.length - 1) Coach.updateProgression(updatedSession);
        this.save();
    },
    deleteSession(index) {
...
const Coach = {
    detectPlateau(exId) {
        ...
        const stalled = (efforts[2].weight <= efforts[1].weight && efforts[1].weight <= efforts[0].weight) &&
                        (efforts[2].reps <= efforts[1].reps && efforts[1].reps <= efforts[0].reps);
        ...
        muscles.forEach(m => {
            ...
            if (!ex || (wristPain && ex.joint === 'wrist')) {
                ...
                if (pool.length > 0) {
                    ...
                    Store.data.activeExercises[m] = ex.id;
                }
            }
...
const UI = {
        ...
        const modalHtml = `
            <div id="readiness-modal" class="modal-overlay">
                <div class="modal-content">
                    ...
                    <p style="color:#666; margin-bottom:20px;">Be honest. The Coach will adjust volume.</p>
                    <div class="readiness-options">
                        <button class="readiness-btn" onclick="UI.confirmReadiness(2)">😫<br><small>Tired</small></button>
                        <button class="readiness-btn" onclick="UI.confirmReadiness(3)">😐<br><small>Okay</small></button>
                        <button class="readiness-btn" onclick="UI.confirmReadiness(5)">💪<br><small>Great</small></button>
                    </div>
                </div>
            </div>`;
        ...
        this.container.innerHTML = `
            <div class="card">
                ...
                <div class="summary-grid">
                    <div class="summary-box"><div class="summary-label">Last Upper</div><div class="summary-val">${lastUp ? formatDate(lastUp.date) : '--'}</div></div>
                    <div class="summary-box"><div class="summary-label">Last Lower</div><div class="summary-val">${lastLow ? formatDate(lastLow.date) : '--'}</div></div>
                </div>
    ...
    confirmReadiness(score) {
        document.getElementById('readiness-modal').classList.remove('active');
        this.startNewSession(this.pendingWorkoutType, score);
    },

    startNewSession(type, readiness) {
        const g = Coach.generateWorkout(type, readiness);
    ...
    renderActiveSession(isResumeOrEdit) {
        ...
        if (isHistoryEdit) {
            ...
            dateHeader = `<div class="card" style="background:#fff3cd; border:1px solid #ffeeba;"><label style="font-size:0.8rem; font-weight:bold;">Editing Date:</label><input type="date" id="edit-date-input" value="${dateVal}" style="margin-bottom:0;"></div>`;
        }
        ...
        const exercisesHtml = this.currentPlan.map((ex, i) => {
            ...
            const setRows = Array.from({length: ex.sets}, (_, k) => k + 1).map(s => {
                ...
                else { repVal = dataMap[`reps-${i}-${s}`] || ''; rirVal = dataMap[`rir-${i}-${s}`] !== undefined ? dataMap[`rir-${i}-${s}`] : 2; }
    ...
    swapExercise(index) {
        ...
        if (newEx && confirm(`Swap ${oldEx.name} for ${newEx.name}?`)) {
            Store.data.activeExercises[oldEx.muscle] = newEx.id; Store.save();
    ...
    startTimer(seconds) {
        ...
        let rem = seconds;
        const tick = () => {
            const m = Math.floor(rem / 60).toString().padStart(2,'0'); const s = (rem % 60).toString().padStart(2,'0'); display.innerText = `${m}:${s}`;
    ...
    scrapeAndSaveDraft() {
        const inputs = {}; document.querySelectorAll('input').forEach(inp => { if (inp.id) inputs[inp.id] = inp.value; });
    ...
    // --- LIBRARY, SETTINGS, CHARTS (Same as before) ---
    renderLib() {
            ...
            if (exercises.length > 0) {
                html += `<h3 class="lib-header">${category}</h3>` + exercises.map(e => `
                    <div class="card clickable" onclick="UI.toggleChart(this, '${e.id}')">
                        <div style="display:flex; justify-content:space-between;"><strong>${e.name}</strong><span style="font-size:0.7rem; background:#eee; padding:2px 6px; border-radius:4px;">${e.muscle}</span></div>
    ...
    toggleChart(card, exId) {
        ...
        else { document.querySelectorAll('.card.expanded').forEach(c => c.classList.remove('expanded')); card.classList.add('expanded'); this.renderChart(exId, container); }
    },
    ...
    renderChart(exId, container) {
        ...
        const vals = data.map(d => d.val); const min = Math.min(...vals) * 0.9; const max = Math.max(...vals) * 1.1; const range = max - min;
    ...
    renderHistoryManager() {
        ...
        const recent = Store.data.history.map((h, i) => ({...h, origIndex: i})).reverse().slice(0, 3);
